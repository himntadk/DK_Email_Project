...
<div class="email-app mb-4">
    ...
    <main class="inbox">
        <div class="toolbar">
            ...
            <div class="btn-group">
               <a href="draft.php"> <button type="button" class="btn btn-light">
        ...
        $id = $_GET['id'];
        $select_draft= "SELECT * FROM draft WHERE Id= '$id' ";
        $query_draft= mysqli_query($con,$select_draft);
        $res= mysqli_fetch_array($query_draft);
        if(empty($res)){
           ?>
            <script>
        ...
        <div class="input-box">
        <input type="text" name="from" value="<?php echo $res['Sender'];?>" required="">
        <label>From</label>
        </div>
        <div class="input-box">
        <input type="text" name="to" value="<?php echo $res['Receiver'];?>" required="">
        <label>To</label>
        </div>
    ...
    <table class="table table-striped">
        <thead>
            <tr>
                <th><input type="checkbox" onClick="select_all()" id="select"> select all</th>
            </tr>
        </thead>
        <tbody>
        <?php
        include '../connectionuju.php';
         ...
         while($res1= mysqli_fetch_array($query_client)){
            ?>
                 <tr>
                <td><input type="checkbox" class="box_check" id='<?php echo $res1['Id'];?>' name="checkbox[]"
                     value="<?php echo $res1['Email'];?>"></td>
        ...
        <div class="input-box">
        <input type="text" name="sub" value=" <?php echo $res['Subject'];?>" required="">
        <label>Subject</label>
        </div>
        <div class="high">

        </div>
        <textarea name="mail" id="mail" placeholder="Write Your Mail Here"><?php echo $res['Mail'];?></textarea>
        <div class="input-box">
    ...
    <a href="#" data-toggle="tooltip" data-placement="bottom" title="make draft"><button name="draft" class="btn btn-light inline">
        ...
        <a href="#" data-toggle="tooltip" data-placement="bottom" title="send"><button name="sendmail" class="btn btn-light" onClick="removedraft('<?php echo $res['Id'];?>')"> <span  class="fa fa-paper-plane"></span></button></a>
...
function up_down(){
  ...
  if (x.style.display === "none") {
    ...
    document.getElementById('up').innerHTML = '<span class="fa fa-caret-square-o-left"></span>';
  } else {
    x.style.display = "none";
    document.getElementById('up').innerHTML = '<span class="fa fa-caret-square-o-right"></span>';
  }
}
function reset(){
...
const box_check = document.querySelectorAll('input[type=checkbox]:checked');
let colors = [];
